 ...
 *  @param {string[]} cmds
 */
 export function urbanDefine(message, cmds) {
   const func = 'urbanDefine';
      ...
      .then( (body) => {
        if (body.result_type == 'no_results') {
          message.channel.send('There are no results for: ' +
                               term.replace('+', ' '))
            .catch( (reason) => {
              log.info(reason, 'No result message');
            });
        } else {
          ...
          message.channel.send('**' + cmds.slice(1).join(' ') + ':**\n' +
                               firstDef.definition + '\n' + firstDef.example)
            .catch( (reason) => {
              log.info(reason, 'Definition message');
            });
        }
      }).catch( (err) => {
        log.warn(err, 'Access API Error');
        message.channel.send('Error accessing Urban Dictionary')
          .catch( (reason) => {
            log.info(reason, 'Reject message');
          });
      });
 }
 ...
 */
 export function getAvatar(message, cmds) {
   const func = 'getAvatar';

   if (cmds[1] == undefined) {
     log.verbose('undefined', 'Getting username');
     message.channel.send('You need to specify who\'s avatar I am looking for!')
      .catch( (reason) => {
        log.info(reason, 'Reject username message');
      });
     return;
   }
   ...
   if (/<@.?\d+>/.test(memberName)) {
     ...
     if (typeof obj !== 'undefined') {
      ...
      log.verbose('', 'Found ' + obj.displayName + '\'s avatar at ' + avatarUrl);
     }
   } else {
     for (let [, memberObj] of message.guild.members) {
       ...
       if (displayName == memberName || username == memberName) {
        ...
        log.verbose('', 'Found ' + memberObj.displayName + '\'s avatar at ' + avatarUrl);
        break;
       }
     }
   }
   ...
   if (avatarUrl) {
     ...
     message.channel.send(avatarUrl)
      .catch( (reason) => {
        log.info(reason,'Reject avatar message');
      });
   } else {
     log.verbose('undefined', 'Could not find member ' + user);
     message.channel.send('I couldn\'t find that member!')
      .catch( (reason) => {
        log.info(reason, 'Reject avatar null');
      });
   }
 }
...
* @param {message} message - A message object as defined in discord.js
*/
 export function coinFlip(message) {
   const func = 'coinFlip';
   ...
   if (flip < 50) {
     ...
     message.channel.send('Uhm... the coin landed on its side, flipping again.')
      .catch( (reason) => {
        log.info(reason, 'Reject side message');
      });
    message.channel.send('!coinflip')
      .catch( (reason) => {
        log.info(reason, 'Reject flip again message');
      });
    return;
   }
   message.channel.send('You flipped **' + coin + '**')
    .catch( (reason) => {
      log.info(reason, 'Rejected flip message');
    });
 }
  ...
  */
 export function rate(message, cmds) {
   const func = 'rate';
   ...
   if (str.toLowerCase() == 'staed') {
     message.channel.send('Staeds are great! I\'ll give Staed a 10/10!')
      .catch( (reason) => {
        log.info(reason, 'Reject rate Staed message');
      });
     log.verbose('', 'Rated a Staed');
   } else if (str.toLowerCase() == 'sawai') {
     message.channel.send('Sawais are :put_litter_in_its_place: ' +
                          'I\'ll give Sawai a 0/10')
      .catch( (reason) => {
        log.info(reason, 'Reject rate Sawai message');
      });
     log.verbose('', 'Rated a Sawai');
   } else {
     log.verbose('', 'Rated ' + str + ' as ' + rating + '/10');
     message.channel.send('I\'d rate ' + str + ' ' + rating + '/10')
      .catch( (reason) => {
        log.info(reason, 'Reject rate message');
      });
   }
 }
 ...
 */
  ...
  */
 export function getOptions(hostname, path, tags) {
   const func = 'getOptions';
   ...
   log.verbose('request', 
               'Received request for: ' + path + tags);
   return options;
 }
  ...
  * @param {message} message - A message object as defined in discord.js
  */
export function deleteBooru(message) {
  ...
  message.channel.fetchMessages({limit: 100})
    .then( (msgs) => {
      ...
      message.channel.send('No booru post to delete in the last 100 messages!')
        .catch( (reason) => {
          log.info(reason, 'Reject delete exhaust');
        });
    })
    .catch( (err) => {
      message.channel.send('Failed to fetch past messages')
        .catch( (reason) => {
          log.info(reason, 'Reject delete not found');
        });
      log.warn('err', 'No messages found');
    });
}
...
//   let dice: string = /(\d+D\d+\s*[\+\-]*\s*)+/i.exec(rollInfo)[0];
//   let diceArray: string[] = dice.replace('-', '+-').split('+');
...
function aniListQuery(queryType, message, title) {
  ...
  if (aniListToken != null && aniListToken.expires > (Date.now()/1000 - 60)) {
    if ('airing' == queryType.toLowerCase()) {
      ...
      log.verbose('invalid', 'Invalid queryType in aniListQuery with token');
      return;
    }
  }
...
function aniListAiring(message) {
  ...
  request(airingOptions)
    .then( (body) => {
      ...
      if (resultString.toString().length > 2000) {
        ...
        message.channel.send('Search results truncated due to length...')
          .catch( (reason) => {
            log.verbose('', 'Truncated season search results');
          });
      }
    ...
    .catch( (reason) => {
      log.info(reason, 'Reject request message');
    });
}
 ...
 * @param {message} message A message object as defined in discord.js
 */
export function season(message) {
  ...
  request(queryOptions)
    .then( (body) => {
      ...
      message.channel.send(text)
        .catch( (reason) => {
          log.info(reason, 'Reject send message');
        });
    })
    .catch( (reason) => {
      log.info(reason, 'Reject request message');
 ...
 * @param {message} message A message object as defined in discord.js
 */
export function aninfo(message) {
 ...
 */
 export function padRight(orig, targetLength) {
   let text = orig;
